...
export class EventStore
  implements IEventPublisher, IMessageSource, OnModuleDestroy, OnModuleInit
{
  ...
  async publish(event: IEvent, stream?: string) {
    ...
    try {
      ...
      if (err.name === 'WrongExpectedVersionError') {
        this.logger.warn(
          'Detect duplicate event ' + eventPayload.type + ' ' + err.message,
        );
  ...
  subscribeToCatchupSubscription(
    ...
    try {
      ...
      this.logger.log(
        `EventStore subcribe to stream ${stream} from ${lastCheckpoint}`,
      );
      ...
      this.logger.error(
        'Something went wrong when subscribe catchup stream',
        err,
      );
    }
  }
  ...
  async createPersistentSubscription(
    ...
    try {
      ...
      const result = await this.eventStore
        .getConnection()
        .createPersistentSubscription(stream, subscriptionName, {
          readBatchSize: 20,
      ...
      const resolved = (await this.eventStore
        ...
        .connectToPersistentSubscription(
            ...
            ),
          {
            ...
            password:
              process.env.EVENT_STORE_CREDENTIALS_PASSWORD || 'changeit',
          },
          20,
          true,
  ...
  async onEvent(
    ...
    let eventType;
    const handler =
      this.eventHandlers[(eventType = rawData.eventType)] ||
      this.eventHandlers[(eventType = event.eventType)];
    if (!handler) {
      this.logger.warn(
        `Received event that could not be handled! ${eventType}`,
      );
      return;
    }
  ...
  onDropped(
    ...
    this.logger.warn(
      `Subscription to dropped with reason ${_reason}`,
      error.message,
    );
